...
program = {
    "Monday": {
        ...
        "audio_opening": "monday_opening.mp3", # Matches filename generated by script
        "exercises": [
            {
...
if page == "Home":
    ...
    **Coach's Tip:** consistency is key. Don't skip a day, don't cheat yourself!  Results guaranteed after 12 weeks.
    ...
    # --- AUDIO COACH SECTION ---
    # This plays the Opening Audio for the day
    st.markdown('<div class="audio-card">', unsafe_allow_html=True)
    st.markdown(f"**🔊 Coach D's Pre-Game Speech ({page})**")
    st.audio(f"{BASE_AUDIO_URL}/{day_data['audio_opening']}")
    st.markdown("Press play to hear today's focus and mindset tips.")
    st.markdown('</div>', unsafe_allow_html=True)
